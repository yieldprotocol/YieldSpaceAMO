...
contract YieldSpaceAMO is Owned {
    ...
    function fraxForNewRate(bytes6 seriesId, uint256 newRate) external view returns (uint256 fraxAmount) {
        ...
        console.log(2);
        int128 rua64 = _denormalize(int128(YieldMath.ONE) + int128(uint128(ru64).pow(a, YieldMath.ONE)), int128(a), int128(YieldMath.ONE));
        console.log("+ + file: YieldSpaceAMO.sol + line 246 + fraxForNewRate + rua64", uint128(rua64));
        console.log(3);
        int128 denominator64 = _denormalize(int128(uint128(rua64).pow(YieldMath.ONE, a)), int128(YieldMath.ONE), int128(a));
        console.log("+ + file: YieldSpaceAMO.sol + line 246 + fraxForNewRate + denominator64", uint128(denominator64));
        console.log(4);
        uint128 xy = (termA + termB);
        console.log(5);
        uint256 numerator = uint256(uint128(int128(xy.pow(YieldMath.ONE, a)).div(uint256(1e18).fromUInt()))); // change from wei to whole frax
